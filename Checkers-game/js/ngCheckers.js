...
angular.module('ngCheckers', [])
  ...
  { 
    var RED = "Rot", 
        ...
        BOARD_WIDTH = 8,
        selectedSquare = null;
    var timeStart, timeEnd, timeDuration = 0;
    ...
    { 
      ...
      {
        return {"backgroundColor": "#FF0000"}; //Die roten Spielsteine werden eingefärbt
      }
      else if (square.player === BLACK)
      {
        return {"backgroundColor":"#A3A3A3"}; //Die schwarzen Spielsteine werden eingefärbt
      }
      return {"backgroundColor": "none"}; //Der Rest erhält keine Farbe
    }
    ...
    $scope.setClass = function(square) //Farbe der Felder wird gesetzt
    {
      if (square.y % 2 === 0) //Zeilen werden geprüft, nur gerade
      {
        ...
        {
          return {"backgroundColor": "white"}; //Falls das Feld die Bewegungsmöglichkeit anzeigt ist es grün, sonst weiß
        } 
        else 
        {
          return {"backgroundColor": square.isChoice ? "green" : "black"}; //Alle ungeraden Spalten werden schwarz
        }
      ...
      else //Nun werden alle ungeraden Zeilen durchlaufen
      {
        ...
        {
          return {"backgroundColor": square.isChoice ? "green" : "black"}; //Alle anderen Felder werden schwarz
        }
      }
    }
    ...
    function submit() {
      ...
      var query = "http://localhost:3520/Create?";
      query += "username=" + username;
      query += "&time=" + timeDuration;

      $.ajax({ url: query, crossDomain: true, dataType: 'json', type: 'GET'})
      .done(function(json) {
        console.log("Spieldauer erfolgreich gespeichert!");
      })
      .fail(function() {
        alert("Fehler beim Speichern der Spieldauer!");
      });
    }

    $scope.readAll = function() {
      var query = "http://localhost:3520/ReadAll?";
      
      $.ajax({ url: query, crossDomain: true, dataType: 'json', type: 'GET' })
      .done(function(json) {
        $scope.highscores = json;
        $scope.$apply();
      })
      .fail(function() {
